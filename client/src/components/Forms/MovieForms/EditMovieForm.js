...
const EditMovieForm = () => {
    ...
    const [newMovie, setNewMovie] = useState({ title: movie.title, year: movie.year, cast: movie.Cast.map(actor => actor.id), producer: movie.Producers.map(producer => producer.id), director: movie.Directors.map(director => director.id) })
    ...
    <div>
        {movie &&
            <div>
                ...
                <form>
                  <div className='edit-field-wrapper' style={{flex: 1, marginRight: '1%'}}>
